...
struct ProgressPathView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showLevelDetail) {
            LevelDetailView(level: selectedLevel, realm: Realm.allRealms[selectedRealm - 1])
                                ...
                                ZStack {
                                    ...
                                    if completedRealms.contains(realmId) {
                                        Image(systemName: "checkmark")
                                            .foregroundColor(Color(hex: Realm.allRealms[realmId - 1].color))
                                    } else {
                                        Text("\(realmId)")
                                            .font(.headline.bold())
                                            .foregroundColor(selectedRealm == realmId ? Color(hex: Realm.allRealms[realmId - 1].color) : .white)
                                    }
                                }
                                
                                Text(Realm.allRealms[realmId - 1].name.prefix(8))
                                    .font(.caption2)
    ...
    // MARK: - Helpers
    var realm: Realm {
        Realm.allRealms[selectedRealm - 1]
    }
    
    var realmTheme: Realm.RealmTheme {
        realm.theme
    }
...
struct LevelDetailView: View {
    ...
    var challengeForLevel: Realm.RealmChallenge {
